...
export const generateMediaDescription = async (file: File): Promise<string> => {
    try {
        ...
        if (file.type.startsWith('image/')) {
            ...
            // For videos, generate based on filename (text only)
            const model = genAI.getGenerativeModel({ model: 'gemini-pro' });
            const result = await model.generateContent(
...
export const uploadOriginalMedia = async (
    ...
    try {
        ...
        // Check file size (6MB limit for standard upload)
        const FILE_SIZE_LIMIT = 6 * 1024 * 1024; // 6MB

        if (file.size > FILE_SIZE_LIMIT) {
            console.log('File is larger than 6MB, using TUS resumable upload');
            await uploadWithTus(file, 'media', filePath);
            ...
            console.log('=== uploadOriginalMedia END (TUS) ===');
...
/**
 * Upload large file using TUS
 */
const uploadWithTus = async (file: File, bucketName: string, filePath: string): Promise<void> => {
    return new Promise(async (resolve, reject) => {
        const { data: { session } } = await supabase.auth.getSession();
        const token = session?.access_token;

        if (!token) {
            reject(new Error('No auth token available for upload'));
            return;
        }
