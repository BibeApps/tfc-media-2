...
const Settings: React.FC = () => {
    ...
    const checkMFAStatus = async () => {
        try {
            ...
            setTfaEnabled(data?.totp && data.totp.length > 0);
        } catch (err) {
        ...
        if (tfaEnabled) {
            ...
            if (confirm('Are you sure you want to disable Two-Factor Authentication?')) {
                try {
                    ...
                    setSaveMessage({ type: 'error', text: err.message || 'Failed to disable 2FA' });
                    setTimeout(() => setSaveMessage(null), 5000);
                }
            }
        } else {
            // Enable 2FA - show enrollment modal
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select value={profileForm.state} onChange={e => setProfileForm({ ...profileForm, state: e.target.value })} className="w-full bg-gray-50 dark:bg-obsidian border border-gray-200 dark:border-white/10 rounded-lg p-2.5 text-gray-900 dark:text-white">
                        ...
                        <div>
                            ...
                            <input type="text" value={profileForm.zip} onChange={e => setProfileForm({ ...profileForm, zip: formatZipCode(e.target.value) })} className="w-full bg-gray-50 dark:bg-obsidian border border-gray-200 dark:border-white/10 rounded-lg p-2.5 text-gray-900 dark:text-white" placeholder="12345" maxLength={5} />
                        </div>
                    </div>
                </div>
