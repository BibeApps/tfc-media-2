                ...
                <div className="p-8 overflow-y-auto space-y-8 bg-gray-50/50 dark:bg-obsidian/50 print:bg-white print:p-6 print:overflow-visible">
                    <div className="flex justify-between items-center print:border-b print:pb-6">
                        <div>
                            ...
                            <p className="font-medium text-gray-900 dark:text-white">{formatDate(order.date)}</p>
                        </div>
                        <div className="text-right">
                    ...
                    <div className="grid grid-cols-2 gap-8 p-6 bg-white dark:bg-charcoal rounded-xl border border-gray-200 dark:border-white/5 print:border-0 print:p-0 print:gap-4">
                        ...
                        <div>
                            ...
                            {user?.address && (
                                <>
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">{user.address}</p>
...
const Purchases: React.FC = () => {
    ...
    // Separate effect to clear success parameter once
    useEffect(() => {
        ...
        try {
            ...
            if (data) {
                const ordersWithCounts = data.map((order: any) => ({
                    ...
                    items: order.order_items?.length || 0, // Count the items array
                    status: order.status,
                    itemsList: []
                }));
                console.log('✅ Processed orders:', ordersWithCounts);
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                                            <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> {formatDate(order.date)}</span>
                                            <span>•</span>
