...
import { ChevronRight, ArrowLeft, ShoppingCart, Lock } from 'lucide-react';
import { EventType, Event, Session, ViewState } from '../types';
...
const Gallery: React.FC = () => {
  ...
  const [history, setHistory] = useState<{type?: EventType, event?: Event, session?: Session}>({});
  ...
  const handleBack = () => {
    if (viewState === ViewState.GRID) setViewState(ViewState.SESSIONS);
    else if (viewState === ViewState.SESSIONS) setViewState(ViewState.EVENTS);
    else if (viewState === ViewState.EVENTS) setViewState(ViewState.TYPES);
  };

  // Breadcrumbs
  const Breadcrumbs = () => (
    <div className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400 mb-8 overflow-x-auto pb-2">
      <button onClick={() => setViewState(ViewState.TYPES)} className="hover:text-electric whitespace-nowrap">Gallery</button>
      {history.type && (
        <>
          <ChevronRight className="w-4 h-4 flex-shrink-0" />
          <button onClick={() => setViewState(ViewState.EVENTS)} className={`hover:text-electric whitespace-nowrap ${viewState === ViewState.EVENTS ? 'text-gray-900 dark:text-white font-bold' : ''}`}>
            {history.type.name}
          </button>
        </>
      )}
      {history.event && viewState !== ViewState.EVENTS && viewState !== ViewState.TYPES && (
        <>
          <ChevronRight className="w-4 h-4 flex-shrink-0" />
          <button onClick={() => setViewState(ViewState.SESSIONS)} className={`hover:text-electric whitespace-nowrap ${viewState === ViewState.SESSIONS ? 'text-gray-900 dark:text-white font-bold' : ''}`}>
            {history.event.name}
          </button>
        </>
      )}
      {history.session && viewState === ViewState.GRID && (
        <>
          <ChevronRight className="w-4 h-4 flex-shrink-0" />
          <span className="text-gray-900 dark:text-white font-bold whitespace-nowrap">{history.session.name}</span>
        </>
      )}
    </div>
  );
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      {viewState === ViewState.TYPES && (
        ...
        >
          {galleryData.map((type) => (
            <div key={type.id} onClick={() => handleSelectType(type)} className="group cursor-pointer bg-white dark:bg-charcoal rounded-2xl overflow-hidden border border-gray-200 dark:border-white/5 hover:border-electric/50 transition-all shadow-sm hover:shadow-xl hover:shadow-electric/10 flex flex-col">
              <div className="h-40 overflow-hidden bg-gray-100 dark:bg-black/50">
                {type.thumbnail ? (
                  <img src={type.thumbnail} alt={type.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                ) : (
                   <div className="w-full h-full flex items-center justify-center text-gray-400 dark:text-gray-600">No Image</div>
                )}
              </div>
              <div className="p-5 flex-1 flex flex-col">
                <h3 className="text-xl font-heading font-bold mb-2 text-gray-900 dark:text-white group-hover:text-electric transition-colors">{type.name}</h3>
                <p className="text-gray-600 dark:text-gray-400 text-sm mb-4 line-clamp-2 flex-1">{type.description}</p>
      ...
      {/* VIEW: EVENTS */}
      {viewState === ViewState.EVENTS && history.type && (
        <motion.div 
          initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          className="space-y-6"
        >
          <h2 className="text-3xl font-heading font-bold mb-6 text-gray-900 dark:text-white">{history.type.name} Events</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {history.type.events.map((event) => (
      ...
      {/* VIEW: SESSIONS */}
      {viewState === ViewState.SESSIONS && history.event && (
        <motion.div 
          initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        >
          <h2 className="text-3xl font-heading font-bold mb-2 text-gray-900 dark:text-white">{history.event.name}</h2>
          <p className="text-gray-600 dark:text-gray-400 mb-8">Select a session to view photos</p>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {history.event.sessions.map((session) => (
              <div key={session.id} onClick={() => handleSelectSession(session)} className="relative group cursor-pointer rounded-xl overflow-hidden aspect-video bg-white dark:bg-charcoal shadow-sm hover:shadow-md border border-gray-200 dark:border-white/5 hover:border-electric/50">
                ...
                <div className="absolute inset-0 bg-black/50 group-hover:bg-black/40 transition-colors flex items-center justify-center">
                  <div className="text-center p-4">
                    ...
                    <p className="text-gray-200 text-xs drop-shadow-sm">{session.items.length} Items</p>
                  </div>
                </div>
              </div>
            ))}
             {history.event.sessions.length === 0 && (
              <div className="col-span-full text-center py-12 text-gray-500 bg-white dark:bg-charcoal rounded-xl border border-gray-200 dark:border-white/5">
      ...
      {/* VIEW: GRID ITEMS */}
      {viewState === ViewState.GRID && history.session && (
        <motion.div 
          initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        >
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
            <div>
              <h2 className="text-3xl font-heading font-bold mb-1 text-gray-900 dark:text-white">{history.session.name}</h2>
              <p className="text-gray-600 dark:text-gray-400">{history.session.date}</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {history.session.items.map((item) => (
            ...
            {history.session.items.length === 0 && (
