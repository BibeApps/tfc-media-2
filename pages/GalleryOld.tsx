...
const Gallery: React.FC = () => {
  ...
  const fetchEvents = async (categoryId: string) => {
    try {
      setLoading(true);
      // RLS policies will filter events based on client email
      const { data, error } = await supabase
        .from('events')
        .select('*')
  ...
  const fetchSessions = async (eventId: string) => {
    try {
      ...
      // Filter by email for non-admin users
                ...
                events.map((event) => (
                  <div key={event.id} onClick={() => handleSelectEvent(event)} className="flex flex-col bg-white dark:bg-charcoal rounded-xl overflow-hidden border border-gray-200 dark:border-white/5 cursor-pointer hover:bg-gray-50 dark:hover:bg-white/5 transition-colors shadow-sm hover:shadow-md hover:border-electric/30 group">
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <p className="text-gray-500 dark:text-gray-400 text-xs mb-4">{formatDate(event.date)}</p>
      ...
      {viewState === ViewState.SESSIONS && selectedEvent && (
        ...
        >
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {sessions.length === 0 ? (
                <div className="col-span-full text-center py-12 bg-white dark:bg-charcoal rounded-xl border border-gray-200 dark:border-white/5">
                  {user && user.role !== 'admin' ? (
                    <div className="space-y-2">
                      <p className="text-gray-600 dark:text-gray-400">No media has been uploaded for your projects yet.</p>
                      <p className="text-sm text-gray-500 dark:text-gray-500">Your photos and videos will appear here once they're ready.</p>
                    </div>
                  ) : (
                    <p className="text-gray-500">No sessions found for this event.</p>
                  )}
                </div>
              ) : (
                  ...
                  <div key={session.id} onClick={() => handleSelectSession(session)} className="relative group cursor-pointer rounded-xl overflow-hidden aspect-video bg-white dark:bg-charcoal shadow-sm hover:shadow-md border border-gray-200 dark:border-white/5 hover:border-electric/50">
                    ...
                    <div className="absolute inset-0 bg-black/50 group-hover:bg-black/40 transition-colors flex items-center justify-center">
                      <div className="text-center p-4">
                        ...
                        <p className="text-gray-200 text-xs drop-shadow-sm">{formatDate(session.date)}</p>
                      </div>
                    </div>
                  </div>
                ))
        ...
        >
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">{formatDate(selectedSession.date)}</p>
            </div>
