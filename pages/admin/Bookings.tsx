...
const Bookings: React.FC = () => {
    ...
    const updateBookingStatus = async (id: string, status: BookingStatus) => {
        try {
            ...
            if (booking && ['confirmed', 'completed', 'cancelled'].includes(status)) {
                try {
                    // Format the date for email — parse YYYY-MM-DD manually to avoid
                    // UTC timezone shift (new Date("2026-03-15") → UTC midnight → off by one day in US timezones)
                    ...
                    const formattedDate = formatEmailDate(booking.booking_date);
                    ...
                    if (status === 'confirmed') {
                        ...
                        bodyMessage = 'Great news! Your booking has been confirmed.';
                    } else if (status === 'completed') {
                        subject = 'Booking Completed!';
                        headerColor = 'linear-gradient(135deg, #0EA5E9 0%, #0284C7 100%)';
                        headerText = '🎉 Booking Completed!';
                        bodyMessage = 'Your booking has been completed. Thank you for choosing TFC Media!';
                    } else if (status === 'cancelled') {
                        subject = 'Booking Cancelled';
                        headerColor = 'linear-gradient(135deg, #EF4444 0%, #DC2626 100%)';
                        headerText = '❌ Booking Cancelled';
                        bodyMessage = 'Your booking has been cancelled. If you have any questions, please contact us.';
                    }

                    // Send status update email directly to booking email (no account required)
                            ...
                            html: `
                                ...
                                <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                                    ...
                                    <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
                                        ...
                                        <p style="margin-top: 30px;">${status === 'cancelled' ? 'If you did not request this cancellation, please contact us immediately.' : 'Thank you for choosing TFC Media!'}</p>
