...
const Clients: React.FC = () => {
    ...
    const fetchClients = async () => {
        try {
            ...
            // Fetch all orders to calculate total spent per client
            // Only count paid/completed orders, exclude pending/refunded
            ...
            const clientsWithTotals = (profilesData || []).map(client => {
                const clientOrders = ordersData?.filter(order => order.client_id === client.id) || [];
                const totalSpent = clientOrders.reduce((sum, order) => sum + (order.total_amount || 0), 0);
    ...
    const handleResetPassword = async (client: Client) => {
        if (!window.confirm(`Reset password for ${client.name}? They will receive an email with a temporary password.`)) return;
    ...
    return (
        <div className="space-y-6">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => setAddUserData({ ...addUserData, zip: formatZipCode(e.target.value) })}
                                                        className="w-full px-4 py-2 bg-gray-50 dark:bg-obsidian border border-gray-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-electric"
                                                        placeholder="12345"
                                                        maxLength={5}
                                                    />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="text"
                                            value={formData.zip || ''}
                                            onChange={(e) => setFormData({ ...formData, zip: formatZipCode(e.target.value) })}
                                            className="w-full px-4 py-2 bg-gray-50 dark:bg-obsidian border border-gray-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-electric"
                                            placeholder="12345"
                                            maxLength={5}
                                        />
            ...
            <AnimatePresence>
                {isResettingPassword && selectedClient && resetPassword && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                        <motion.div
                            initial={{ opacity: 0, scale: 0.95 }}
                            animate={{ opacity: 1, scale: 1 }}
                            ...
                            <div className="p-6 space-y-6">
                                <div className="text-center">
                                    ...
                                    <h4 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                                        Password Reset for {selectedClient.name}
                                    </h4>
                                    <p className="text-gray-600 dark:text-gray-400 mb-6">
                                        An email with the temporary password has been sent to <strong>{selectedClient.email}</strong>
                                    </p>
                                </div>
