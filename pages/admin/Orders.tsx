    ...
    const fetchOrders = async () => {
        try {
            const { data, error } = await supabase
                ...
                .select(`
          *,
          profiles:client_id (name, email, address, city, state, zip)
        `)
            ...
            const mappedOrders = (data || []).map((order: any) => ({
                ...
                clientAddress: order.profiles?.address || '',
                clientCity: order.profiles?.city || '',
                clientState: order.profiles?.state || '',
                clientZip: order.profiles?.zip || '',
            }));
                ...
                <div className="p-8 overflow-y-auto space-y-8 bg-gray-50/50 dark:bg-obsidian/50 print:bg-white print:p-6 print:overflow-visible">
                    ...
                    <div className="grid grid-cols-2 gap-8 p-6 bg-white dark:bg-charcoal rounded-xl border border-gray-200 dark:border-white/5 print:border-0 print:p-0 print:gap-4">
                        ...
                        <div>
                            ...
                            {order.clientAddress && (
                                <>
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">{order.clientAddress}</p>
