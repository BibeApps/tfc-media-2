...
import { EventCategory, Event, Session, GalleryItem, PortalProject } from '../../types';
...
const GalleryManager: React.FC = () => {
    ...
    const fetchProjects = async () => {
        try {
            ...
            // Map to PortalProject interface
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className={`bg-white dark:bg-charcoal rounded-xl border border-gray-200 dark:border-white/10 p-6 ${!selectedCategoryId ? 'opacity-50' : ''}`}>
                    ...
                    >
                        ...
                        {events.map(evt => (
                            <option key={evt.id} value={evt.id}>{evt.name.replace(/\s*[-–]\s*\d{1,2}\/\d{1,2}\/\d{4}.*$/, '')}</option>
                        ))}
                    </select>
                ...
                <div className={`bg-white dark:bg-charcoal rounded-xl border border-gray-200 dark:border-white/10 p-6 ${!selectedEventId ? 'opacity-50' : ''}`}>
                    ...
                    >
                        ...
                        {sessions.map(sess => (
                            <option key={sess.id} value={sess.id}>{sess.name} ({formatDate(sess.date)})</option>
                        ))}
                    </select>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder="Search projects by ID, name, or email..."
                                value={projectSearch}
                                onChange={(e) => {
