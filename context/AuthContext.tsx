...
interface AuthContextType {
  ...
  signup: (userData: { name: string; email: string; password: string; company?: string; role: 'admin' | 'client' }) => Promise<{ success: boolean; message?: string }>;
  logout: () => void;
...
// Define Demo Users for robust fallback
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const mapUser = (data: any): ClientUser => ({
    ...
    status: 'active', // profiles table doesn't always have status, default to active
    totalSpent: data.total_spent || 0,
    memberSince: data.member_since,
  });

  const checkSession = async () => {
    // 1. Check for Demo Sessions first (Fail-safe for non-db users)
  ...
  const signup = async (userData: { name: string; email: string; password: string; company?: string; role: 'admin' | 'client' }) => {
      if (!isConfigured) {
          return { success: false, message: 'Database not configured. Cannot create new users.' };
      }
      ...
      try {
          ...
          if (data.user) {
             return { success: true, message: 'Account created successfully! Please check your email or log in.' };
          }
  ...
  const updateProfile = async (id: string, updates: Partial<ClientUser>) => {
    // Handling Demo Mode
    if (id === 'demo-admin' || id === 'demo-client') {
  ...
  return (
    <AuthContext.Provider value={{ user, login, signup, logout, requestPasswordReset, resetPassword, updateProfile, isAuthenticated: !!user, loading }}>
      {!loading && children}
