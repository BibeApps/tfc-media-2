...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [convenienceFeeRate, setConvenienceFeeRate] = useState(0);
  const { user } = useAuth();
  ...
  // Load site settings for tax and fee rates
  useEffect(() => {
    loadSiteSettings();
  }, []);
  ...
  const loadSiteSettings = async () => {
    try {
      const { data, error } = await supabase
        .from('site_settings')
        .select('tax_rate, convenience_fee_rate')
        .single();
      ...
      if (data) {
        ...
        setConvenienceFeeRate(data.convenience_fee_rate ?? 0);
      }
    } catch (err) {
  ...
  const tax = subtotal * (taxRate / 100); // taxRate is stored as percentage (e.g., 7.5 for 7.5%)
  const fee = subtotal * (convenienceFeeRate / 100); // convenienceFeeRate is stored as percentage
  const total = subtotal + tax + fee;

  return (
    <CartContext.Provider value={{ items, addItem, removeItem, clearCart, subtotal, tax, fee, total, isOpen, setIsOpen, loading }}>
      {children}
